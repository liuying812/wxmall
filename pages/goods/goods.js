...
Page({
  ...
  getCheckedSpecId: function () {
    let checkedValueId = this.getCheckedSpecValue().map(function (v) {
      return v.valueId;
    });
  ...
  changeSpecInfo: function () {
    ...
    if (this.isCheckedAllSpec()) {
      ...
      // 规格所对应的货品选择以后
      let checkedProductArray = this.getCheckedProductItem(this.getCheckedSpecId());
  ...
  getCheckedProductItem: function (key) {
    return this.data.productList.filter(function (v) {
      console.info("v="+v);
      console.info(v);
      console.info("v.goodsSpecificationIds..join()=" + v.goodsSpecificationIds.join());
      console.info("key=" + key);
      console.info(v.goodsSpecificationIds.join(",") == key.toString());

      if (v.goodsSpecificationIds.join(",") == key.toString()) {
        return true;
      } else {
        return false;
      }
  ...
  addToCart: function () {
    ...
    if (this.data.openAttr == false) {
      ...
      //根据选中的规格，判断是否有对应的sku信息
      let checkedProductArray = this.getCheckedProductItem(this.getCheckedSpecId());
      console.info("===checkedProductArray==");
      console.info(checkedProductArray);
      if (!checkedProductArray || checkedProductArray.length <= 0) {
        //找不到对应的product信息，提示没有库存
        wx.showToast({
      ...
      //添加到购物车
      util.request(api.CartAdd, { goodsId: this.data.goods.id, number: this.data.number, productId: checkedProduct.id,goodsSpecVals: this.data.checkedSpecText }, "POST")
        .then(function (res) {
          let _res = res;
          if (_res.code == 0) {
            wx.showToast({
